...
class PWControl(object):
            ...
            if c.schedule != None:
                if c.schedule.name in self.schedulebyname:
                    ...
                    if sched != c.schedule._watt:
                        info("apply_schedule_changes: schedule changed. Update in circle %s - %s" % (
                        c.name, c.schedule.name))
    ...
    def apply_control_to_circle(self, control, mac, force=False):
        ...
        c = self.circles[self.bymac[mac]]
        debug('circle mac: %s before1 - state [r,sw,sc] %s %s %s - scname %s' % (
        mac, c.relay_state, control['switch_state'], control['schedule_state'], control['schedule']))
        debug('circle mac: %s before2 - state [r,sw,sc] %s %s %s' % (
        c.mac, c.relay_state, c.switch_state, c.schedule_state))
        ...
        debug('circle mac: %s after1 - state [r,sw,sc] %s %s %s - scname %s' % (
        mac, c.relay_state, control['switch_state'], control['schedule_state'], control['schedule']))
        debug('circle mac: %s after2 - state [r,sw,sc] %s %s %s' % (
        c.mac, c.relay_state, c.switch_state, c.schedule_state))
    ...
    def log_recording(self, control, mac):
        ...
        if control['savelog'].lower() == 'yes':
            ...
            if last_dt == None:
                ...
                debug("start with first %d, last %d, idx %d, last_dt %s" % (
                first, last, idx, last_dt.strftime("%Y-%m-%d %H:%M")))
                ...
                for log_idx in range(first, last + 1):
                    ...
                    for i, (dt, watt, watt_hour) in enumerate(buffer):
                        if i >= idx and not dt is None and dt >= last_dt:
                            ...
                            info("circle buffers: %s %d %s %d %d" % (
                            mac, log_idx, dt.strftime("%Y-%m-%d %H:%M"), watt, watt_hour))
                            debug("proce with first %d, last %d, idx %d, last_dt %s" % (
                            first, last, idx, last_dt.strftime("%Y-%m-%d %H:%M")))
                            last_dt = dt
            ...
            debug("end   with first %d, last %d, idx %d, last_dt %s" % (
            first, last, idx, last_dt.strftime("%Y-%m-%d %H:%M")))
            ...
            for dt, watt, watt_hour in log:
                if not dt is None:
                    ...
                    # debug("MQTT put value in qpub")
                    msg = str('{"typ":"pwenergy","ts":%s,"mac":"%s","power":%s,"energy":%s,"interval":%d}' % (
                    ts_str, mac, watt.strip(), watt_hour.strip(), c.interval))
...
try:
    ...
    if not mqtt:
        ...
        if 'mqtt_user' in cfg and 'mqtt_password' in cfg:
            mqttclient = MqttClient(cfg['mqtt_ip'], cfg['mqtt_port'], qpub, qsub, "Plugwise-2-py", cfg['mqtt_user'],
                                    cfg['mqtt_password'])
        else:
            mqttclient = MqttClient(cfg['mqtt_ip'], cfg['mqtt_port'], qpub, qsub, "Plugwise-2-py")
