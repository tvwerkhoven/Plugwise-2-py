...
class PlugwiseMessage(object):
    ...
    def serialize(self):
        ...
        logcomm("SEND %4d ---> %4s           %16s %s %4s <---" % (
        len(full_msg), self.ID.decode(), self.mac.decode(), args.decode(), checksum.decode()))
        return full_msg
...
class PlugwiseResponse(PlugwiseMessage):
    ...
    def unserialize(self, response):
        ...
        debug("STRU      " + header + " " + logf(self.function_code) + " " + logf(
            self.command_counter) + " <data> " + logf(crc) + " " + footer)
        ...
        if self.expected_command_counter is not None and self.expected_command_counter != self.command_counter:
            raise OutOfSequenceException("expected seqnr %s, received seqnr %s - this may be a duplicate message" % (
            logf(self.expected_command_counter), logf(self.command_counter)))
        if self.ID != 'FFFF' and self.function_code != self.ID:
            raise UnexpectedResponse(
                "expected response code %s, received code %s" % (logf(self.ID), logf(self.function_code)))
        if raw_msg_len != len(self):
            raise UnexpectedResponse(
                "response doesn't have expected length. expected %d bytes got %d" % (len(self), raw_msg_len))
...
class PlugwiseAckResponse(PlugwiseResponse):
    ...
    def unserialize(self, response):
        try:
            ...
            if self.function_code is not None and self.function_code in ['0006', '0061']:
                ...
                raise OutOfSequenceException(
                    "expected command ack from stick. received message with seqnr %s - this may be a duplicate message" % (
                    logf(self.command_counter),))
            else:
                raise
...
class PlugwiseQueryCirclePlusResponse(PlugwiseResponse):
    ...
    def __init__(self, seqnr=None):
        ...
        self.params += [self.channel, self.source_mac_id, self.extended_pan_id, self.unique_network_id,
                        self.new_node_mac_id, self.pan_id, self.idx]
...
class PlugwiseJoinNodeRequest(PlugwiseRequest):
    ...
    def serialize(self):
        ...
        checksum = self.calculate_checksum(msg)
        full_msg = self.PACKET_HEADER + msg + checksum + self.PACKET_FOOTER
        logcomm("SEND %4d ---> %4s        %2s %16s %4s <---" % (
        len(full_msg), self.ID.decode(), args.decode(), self.mac.decode(), checksum.decode()))
        return full_msg
