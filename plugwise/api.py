...
class Stick(SerialComChannel):
    ...
    def __str__(self):
        objetc_str = ""
        objetc_str += "_devtype: " + str(self._devtype)
        objetc_str += "\npan: " + str(self.pan)
        objetc_str += "\nshort_pan: " + str(self.short_pan)
        objetc_str += "\nmac: " + str(self.mac)
        objetc_str += "\nlast_counter: " + str(self.last_counter)
        objetc_str += "\nunjoined: " + str(self.unjoined)
        objetc_str += "\ncircleplusmac: " + str(self.circleplusmac)
        ...
        while 1:
            try:
                ...
                if self.is_in_sequence(resp, seqnr) and (src_mac is None or src_mac == resp.mac):
                    return resp
                error("expect_response: received response but did not return as expected %s %s %s %s" % (
                    str(seqnr), str(src_mac), str(resp.mac), self.is_in_sequence(resp, seqnr)))
                ...
                try:
                    if resp.function_code == b'0000' and not issubclass(resp.__class__, PlugwiseAckResponse):
                        ...
                        if (len(msg) == 22 and msg[0:1] == b'\x05') or (len(msg) == 23 and msg[0:1] == b'\x83'):
                            ...
                            logcomm(
                                "RERR %4d %s - <!> unexpected response error: %s" % (len(msg), logf(msg), str(reason)))
                        # except ProtocolError as reason:
                        #     #retry to receive the response
                        ...
                        logcomm("RERR %4d %s - <!> unexpected response error while expecting Ack: %s" % (
                            len(msg), logf(msg), str(reason)))
...
class Circle(object):
    ...
    def __str__(self):
        ...
        objetc_str += "\nname: " + str(self.name)
        objetc_str += "\nalways_on: " + str(self.always_on)
        objetc_str += "\nlocation: " + str(self.location)
        objetc_str += "\nreverse_pol: " + str(self.reverse_pol)
        objetc_str += "\nproduction: " + str(self.production)
        objetc_str += "\nloginterval: " + str(self.loginterval)

        objetc_str += "\ndevtype: " + str(self._devtype)

        objetc_str += "\ngain_a: " + str(self.gain_a)
        objetc_str += "\ngain_b: " + str(self.gain_b)
        objetc_str += "\noff_noise: " + str(self.off_noise)
        objetc_str += "\noff_tot: " + str(self.off_tot)

        objetc_str += "\nscheduleCRC: " + str(self.scheduleCRC)
        objetc_str += "\nschedule: " + str(self.schedule)

        objetc_str += "\njoined: " + str(self.joined)
        objetc_str += "\nonline: " + str(self.online0)
        objetc_str += "\nonline_changed: " + str(self.online_changed)
        objetc_str += "\npong: " + str(self.pong)
        objetc_str += "\ninitialized: " + str(self.initialized)
        objetc_str += "\nrelay_state: " + str(self.relay_state)
        objetc_str += "\nswitch_state: " + str(self.switch_state)
        objetc_str += "\nschedule_state: " + str(self.schedule_state)
        objetc_str += "\nrequid: " + str(self.requid)
        objetc_str += "\nschedule_state: " + str(self.schedule_state)
        objetc_str += "\nlast_seen: " + self.last_seen
        objetc_str += "\nlast_log: " + str(self.last_log)
        objetc_str += "\nlast_log_idx: " + str(self.last_log_idx)
        objetc_str += "\nlast_log_ts: " + str(self.last_log_ts)
        objetc_str += "\ncum_energy: " + str(self.cum_energy)

        objetc_str += "\npower: " + str(self.power)
        objetc_str += "\npower_ts: " + str(self.power_ts)
    ...
    def _expect_response(self, response_class, seqnr):
        ...
        while retry_count >= 0:
            ...
            if not isinstance(resp, response_class):
                ...
                if resp.status.value == 0xE1:
                    debug(
                        "Received an error status '%04X' from circle '%s' - Network slow or circle offline - Retry receive ..." % (
                            resp.status.value, self.name))
                    # retry_timeout = 1 #allow 1+1 seconds for timeout after an E1.
                    if self.online:
                        info("OFFLINE Circle '%s'." % (self.name,))
                    self.online = False
                    ...
                    error("Received an error status '%04X' from circle '%s' with correct seqnr - Retry receive ..." % (
                        resp.status.value, self.name))
            else:
    ...
    def switch(self, on):
        ...
        if on == True:
            ...
            if resp.status.value != 0xDE:
                error("Wrong switch status reply when switching off. expected '00DE', received '%04X'" % (
                    resp.status.value,))
    ...
    def schedule_onoff(self, on):
        ...
        if on == True:
            ...
            if resp.status.value != 0xE5:
                error("Wrong schedule status reply when setting schedule off. expected '00E5', received '%04X'" % (
                    resp.status.value,))
            self.schedule_state = 'off'
        return
