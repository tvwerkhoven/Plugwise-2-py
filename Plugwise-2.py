...
locnow = datetime.datetime.utcnow() - timedelta(seconds=time.timezone)
now = locnow
...
class PWControl(object):
        ...
        with open(self.lastlogfname, 'a+') as f:
            ...
            for line in f:
                ...
                if len(parts) >= 4:
                    ...
                    ts = int(parts[3])
                logaddr = int(logaddr)
                debug("mac -%s- logaddr -%s- logaddr_idx -%s- logaddr_ts -%s- cum_energy -%s-" % (
                    mac, logaddr, idx, ts, cum_energy))
                try:
        ...
        if control['savelog'].lower() == 'yes':
                ...
                if last_dt is None:
                    if first > 0:
                        ...
                        if len(powlist) < 4:
                            ...
                            error(
                                "log_recording: first time history reading: history entry not complete at first %d, cur %d" % (
                                    first, c_info['last_logaddr']))
            ...
            for dt, watt, watt_hour in log:
                if dt is not None:
                    ...
                    # debug("MQTT put value in qpub")
                    msg = str(
                        '{"typ":"pwenergy","ts":%s,"mac":"%s","power":%s,"energy":%s,"cum_energy":%.4f,"interval":%d}' % (
                            ts_str, mac, watt.strip(), watt_hour.strip(), c.cum_energy, c.interval))
                    qpub.put((self.ftopic("energy", mac), msg, True))
            if f is not None:
                f.close()
    ...
    def connect_unknown_nodes(self):
        for newnodemac in self.device.unjoined:
            ...
            self.device.join_node(newnodemac, newnode is not None)
        # clear the list
    ...
    def run(self):
        ...
        while 1:
            ...
            for c in self.circles:
                if c.online and c.schedule is not None and c.schedule.dst != time.localtime().tm_isdst:
...
try:
    ...
    if not mqtt:
        ...
        if 'mqtt_user' in cfg and 'mqtt_password' in cfg:
            mqttclient = MqttClient(cfg['mqtt_ip'],
                                    cfg['mqtt_port'],
                                    qpub,
                                    qsub,
                                    "plugwise2mqtt",
