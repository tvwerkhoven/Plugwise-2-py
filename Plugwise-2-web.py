...
if not mqtt:
    ...
    if 'mqtt_user' in cfg and 'mqtt_password' in cfg:
        mqttclient = MqttClient(cfg['mqtt_ip'], cfg['mqtt_port'], qpub, qsub, "Plugwise-2-web", cfg['mqtt_user'],
                                cfg['mqtt_password'])
    else:
        mqttclient = MqttClient(cfg['mqtt_ip'], cfg['mqtt_port'], qpub, qsub, "Plugwise-2-web")
...
class PW2PYwebHandler(HTTPWebSocketsHandler):
    ...
    def do_GET(self):
        ...
        if path == '/schedules':
            ...
            schedules = [os.path.splitext(os.path.basename(x))[0] for x in
                         glob.glob(os.curdir + os.sep + 'schedules' + os.sep + '*.json')]
            # print schedules
        ...
        # only allow certain file types to be retrieved
                 ...
                 ('.ws', '.html', '.js', '.css', '.png', '.jpg', '.gif', '.svg', '.ttf', '.woff', '.txt', '.map',
                  '.json')):
